...
then 
	...
	wget https://raw.githubusercontent.com/StevenBlack/hosts/master/alternates/fakenews-gambling-porn/hosts
	wget https://hosts-file.net/ad_servers.txt && sed -i -e 's/^/0.0.0.0/' ad_servers.txt
...
then
	...
	wget http://hosts-file.malwareteks.com/hosts.txt -O hosts && sed -i -e 's/^/0.0.0.0/' hosts
	wget raw.githubusercontent.com/ZeroDot1/CoinBlockerLists/master/hosts -O coinblocker
	wget https://hosts-file.net/hphosts-partial.txt && sed -i -e 's/^/0.0.0.0/' hphosts-partial.txt
	wget http://sysctl.org/cameleon/hosts -O cameleonhosts
...
then 
	...
	wget https://raw.githubusercontent.com/StevenBlack/hosts/master/alternates/fakenews-gambling-porn/hosts
	wget http://hosts-file.malwareteks.com/hosts.txt -O hphosts && sed -i -e 's/^/0.0.0.0/' hphosts
	wget https://hosts-file.net/hphosts-partial.txt && sed -i -e 's/^/0.0.0.0/' hphosts-partial.txt
...
then
	...
	wget hosts-file.net/ad_servers.txt && sed -i -e 's/^/0.0.0.0/' ad_servers.txt
	wget someonewhocares.org/hosts/hosts
	wget raw.githubusercontent.com/ZeroDot1/CoinBlockerLists/master/hosts -O coinblocker
	wget http://sysctl.org/cameleon/hosts -O cameleonhosts
	wget http://winhelp2002.mvps.org/hosts.txt -O MVPShosts
...
sort hosts | uniq -u > /tmp/hosts.new && mv /tmp/hosts.new hosts
