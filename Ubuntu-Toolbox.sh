...
InstallAndConquer(){
	...
	do
		...
		case $software in
			...
			then
				wget https://downloads.vivaldi.com/stable/vivaldi-stable_4.0.2312.38-1_amd64.deb; sudo apt install -f
			elif [[ $browser == 8 ]];
			then
			...
			then
				wget https://cdn.waterfox.net/releases/linux64/installer/waterfox-G3.2.3.en-US.linux-x86_64.tar.bz2; waterfox-G3.2.3.en-US.linux-x86_64.tar.bz2; sudo ln -s ~/waterfox/waterfox /usr/bin/waterfox; sudo mv waterfox /opt && sudo ln -s /opt/waterfox/waterfox /usr/bin/waterfox
