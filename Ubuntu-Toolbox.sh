...
InstallAndConquer(){
	...
	do
		...
		case $software in
			...
			then
				wget https://downloads.vivaldi.com/stable/vivaldi-stable_5.4.2753.37-1_amd64.deb; sudo dpkg -i *.deb && sudo apt install -f
			elif [[ $browser == 8 ]];
			then
			...
			then
				wget https://github.com/WaterfoxCo/Waterfox/releases/download/G4.1.4/waterfox-G4.1.4.en-US.linux-x86_64.tar.bz2; tar -xf waterfox-G4.1.4.en-US.linux-x86_64.tar.bz2; sudo ln -s ~/waterfox/waterfox /usr/bin/waterfox; sudo mv waterfox /opt && sudo ln -s /opt/waterfox/waterfox /usr/bin/waterfox
			...
			then
				mkdir AppImages
				wgethttps://gitlab.com/api/v4/projects/24386000/packages/generic/librewolf/103.0.2-1/LibreWolf.x86_64.AppImage ; chmod +x LibreWolf.x86_64.AppImage; mv LibreWolf.x86_64.AppImage ~/AppImages
