...
InstallAndConquer(){
	...
	do
		...
		case $software in
			...
			then
				wget https://downloads.vivaldi.com/stable/vivaldi-stable_5.5.2805.35-1_amd64.deb; sudo dpkg -i *.deb && sudo apt install -f
			elif [[ $browser == 8 ]];
			then
			...
			then
				wget https://github.com/WaterfoxCo/Waterfox/releases/download/G5.0/waterfox-G5.0.en-US.linux-x86_64.tar.bz2; tar -xf waterfox-G5.0.en-US.linux-x86_64.tar.bz2; sudo ln -s ~/waterfox/waterfox /usr/bin/waterfox; sudo mv waterfox /opt && sudo ln -s /opt/waterfox/waterfox /usr/bin/waterfox
			...
			then
				wget https://github.com/ciderapp/cider-releases/releases/download/v1.5.7/cider_1.5.7_amd64.deb; sudo dpkg -i *.deb; sudo apt install -f
			elif [[ $player == 7 ]];
			then
