...
InstallAndConquer(){
	...
	do
		...
		case $software in
			...
			then
				wget https://linux.palemoon.org/datastore/release/palemoon-32.2.1.linux-x86_64-gtk3.tar.xz; tar -xvf palemoon-32.2.1.linux-x86_64-gtk3.tar.xz; sudo ln -s ~/palemoon/palemoon /usr/bin/palemoon
			...
			then
				wget https://downloads.vivaldi.com/stable/vivaldi-stable_6.1.3035.111-1_amd64.deb; sudo dpkg -i *.deb && sudo apt install -f
			elif [[ $browser == 8 ]];
			then
			...
			then
				wget https://cdn1.waterfox.net/waterfox/releases/G5.1.9/Linux_x86_64/waterfox-G5.1.9.tar.bz2; tar -xf waterfox-G5.1.9.tar.bz2; sudo ln -s ~/waterfox/waterfox /usr/bin/waterfox; sudo mv waterfox /opt && sudo ln -s /opt/waterfox/waterfox /usr/bin/waterfox
			...
			then
				mkdir AppImages
				wget https://gitlab.com/api/v4/projects/24386000/packages/generic/librewolf/114.0.2-1/LibreWolf.x86_64.AppImage; chmod +x LibreWolf.x86_64.AppImage; mv LibreWolf.x86_64.AppImage ~/AppImages
			...
			if [[ $client == 1 ]];
			then
				sudo apt install -y transmission-gtk; sudo ufw allow transmission && sudo ufw reload
			elif [[ $client == 2 ]];
			then
				sudo apt install -y deluge; sudo ufw allow deluge && sudo ufw reload
			elif [[ $client == 3 ]];
			then
				sudo apt install -y qbittorrent; sudo ufw allow qbittorrent && sudo ufw reload
			fi
			;;
			8)
			echo "1 - Guake"
...
EOF
	...
	sudo rm -rf {.cache/*,.thumbnails/*,~/.local/share/Trash/files/*,~/.nv/*,~/.npm/*,~/.w3m/*,~/.esd_auth,~/.local/share/recently-used.xbel,/tmp/*,/var/tmp/*}; history -c && rm ~/.bash_history
