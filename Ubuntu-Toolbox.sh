...
InstallAndConquer(){
	...
	do
		...
		case $software in
			...
			then
				wget https://downloads.vivaldi.com/stable/vivaldi-stable_5.7.2921.65-1_amd64.deb; sudo dpkg -i *.deb && sudo apt install -f
			elif [[ $browser == 8 ]];
			then
			...
			then
				wget https://cdn1.waterfox.net/waterfox/releases/G5.1.4/Linux_x86_64/waterfox-G5.1.4.tar.bz2; tar -xf waterfox-G5.1.4.tar.bz2; sudo ln -s ~/waterfox/waterfox /usr/bin/waterfox; sudo mv waterfox /opt && sudo ln -s /opt/waterfox/waterfox /usr/bin/waterfox
			...
			then
				mkdir AppImages
				wget https://gitlab.com/api/v4/projects/24386000/packages/generic/librewolf/111.0-3/LibreWolf.x86_64.AppImage; chmod +x LibreWolf.x86_64.AppImage; mv LibreWolf.x86_64.AppImage ~/AppImages
